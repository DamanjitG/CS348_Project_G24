...
def get_top_players():
        ...
        query = """SELECT pid, name, creator, COUNT(DISTINCT(username)) AS total_watchers
        ...
        for row in cur.fetchall():
            ...
            name = row[1]
            creator = row[2]
            total_watchers = row[3]
            thecreator = creator if creator else "NBA"
            out.append({"id": pid, "label": f"{name} ({thecreator})", "name":name, "watchers":total_watchers})
